...
## MAS Types: Conversational *and* Background Agents
...
Our platform currently offers four MAS categories:

1. **Storefront MAS** (runs on the messenger on the storefront) (conversational)
2. **Email MAS** (automated email replies) (conversational)
3. **Content MAS** (blog/content generator that works on a schedule – e.g., weekly SEO posts) (background)
4. **Conversation Insights MAS** (post‑conversation summaries and insights) (background)

Content MAS and Conversation Insights MAS are *background agents*. They don’t chat directly with customers; instead, they create content or analyze finished conversations. You still guide them with instructions (e.g., “Write a 600‑word blog post on eco‑friendly packaging every Monday,” or “After each chat, summarize the main question and tag intent keywords.”), but there’s no dynamic back‑and‑forth. As these MAS evolve, we’ll publish full context/tool lists in the guides section.
...
* The [Context guide](https://lookfor.ai/docs/guides/context) – all dynamic variables by MAS type.
* The [Tools guide](https://lookfor.ai/docs/guides/tools) – every built‑in capability your agents can call.
* The [MAS Overview](https://lookfor.ai/docs/guides/mas‑types) – feature comparison of Storefront, Email, Content, and Conversation Insights MAS.
