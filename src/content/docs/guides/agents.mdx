...
- **[Instructions](/user-manual/guides/prompt-engineeringg#what-are-agent-instructions)** – the system prompt that tells the agent _who it is_ and _how it should respond_.
- **[Handoff Agents](/user-manual/guides/handoffs)** – a list of agents that the current agent can delegate to.
- **[Tools](/user-manual/guides/tools)** – a list of functions or APIs the model can invoke to accomplish a task. Depending on the type of the MAS, lookfor provides a set of useful tools for you to equip an agent of the MAS.
- **[Model Settings](/user-manual/guides/models)**  - The model settings of the agent.
...
<Aside type="caution">
    ...
    [Writing Effective Agent Instructions](/user-manual/guides/prompt-engineering) and [Following Single Responsibility Principle](/user-manual/guides/single-responsibility) are powerful weapons to reduce hallucinations.
...
## Context
    ...
    Agents are equipped with a set of context. Context is a set of variables that are available to the agent. To learn more about context, see the [context](/user-manual/guides/context) guide.
